...
class ModelMonitor:
    ...
    def compute_drift(
        self, current_data: pd.DataFrame, baseline_data: pd.DataFrame | None = None
    ) -> dict[str, float]:
    ...
    def check_drift_threshold(
        self, drift_scores: dict[str, float], threshold: float = 2.0
    ) -> list[str]:
    ...
    def log_metrics(
        self, metrics: MonitoringMetrics, timestamp: str | None = None
    ) -> None:
